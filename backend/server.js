...
app.put('/api/admin/customers/:id/assign', authenticate, authorize('admin'), (req, res) => {
  try {
    const { staffId } = req.body
    // Mock assignment - in real app, update customer's assigned staff
    res.json({ success: true, message: 'Customer assigned to staff successfully' })
  } catch (error) {
    res.status(500).json({ success: false, message: error.message })
  }
})
...
app.post('/api/wallet/verify-recharge', authenticate, (req, res) => {
  try {
    const { amount, paymentMethod, transactionId } = req.body
    
    // Simulate wallet data since we don't have a real database
    const walletData = {
      balance: 1250 + amount, // Add recharge amount to existing balance
      transactions: [
        {
        ...
        // Keep existing mock transactions
        { _id: '1', type: 'credit', amount: 2000, description: 'Wallet Recharge', createdAt: new Date().toISOString() },
        { _id: '2', type: 'debit', amount: 750, description: 'Order Payment - WW001', createdAt: new Date(Date.now() - 86400000).toISOString() }
      ]
    }
...
app.get('/api/invoices/:id/pdf', authenticate, (req, res) => {
  try {
    // Mock PDF generation
...
app.listen(PORT, () => {
  console.log(`🚀 WashWish API Server running on port ${PORT}`);
